...
var FILE_EXTENSION = /\.(gif|jpg|jpeg|tiff|png|doc|docx|pdf|xls|xlt|xlsx|mov|asf|avi|qt|avchd|swf|flv|mpg|mp4|wmv)$/i;
var VIDEOS_EXTENSION = /\.(mov|asf|avi|qt|avchd|swf|flv|mpg|mp4|wmv)$/i;
...
var videosext_arr = ["mov","asf","avi","qt","avchd","swf","flv","mpg","mp4","wmv"];
...
// initialize fileupload default file
 ...
 $('#multi_upload_file_reports').fileupload({
     ...
     send : function (e, data)
     {
         ...
         // if uploaded file has invalid extension
         if (!(PDF_EXTENSION).test(uploadFile.name))
         {
             bootbox.alert('File type '+uploadFile.name+' is not supported pdf File.Supported file types are Pdfs(pdf).');
     ...
     done: function (e, data)
     {
         ...
         { 
             ...
             else if(jQuery.inArray(fileExt,videosext_arr) != -1)
             {
                file.url = SITE_URL+'/img/ic-videos@3x.png';
             }

             var btn_val = 'Delete';
             file_table.append('<tr id="file_'+counter+'" rel="'+file.name+'"><td><input type="text" placeholder="Pdfs Name" id="title'+counter+'" name="report_title'+counter+'" class="form-control required"></td><td><img class="list_image" src="'+file.url+'" /></td><td><a  target="_blank" href="'+file.url+'">'+original_name+'</a></td><td><button type="button" id1="'+counter+'" href="javascript:void(0);" id2="'+file.name+'" id3="'+WWW_ROOT+TEMP_IMG_PATH+file.name+'" class="btn red delete remove_file" rel="'+file.size+'"><i class="fa fa-trash delete_btn"></i><span>'+btn_val+'</span></button></td></tr>');
  ...
  /*$('#multi_upload_file_reports').fileupload({
     ...
     send : function (e, data)
     {
         ...
         // if uploaded file has invalid extension
         if (!(VIDEOS_EXTENSION).test(uploadFile.name))
         {
             bootbox.alert('File type '+uploadFile.name+' is not supported video File.Supported file types are Videos(mov|asf|avi|qt|avchd|swf|flv|mpg|mp4|wmv).');
     ...
     done: function (e, data)
     {
         ...
         { 
             ...
             var btn_val = 'Delete';
             file_table.append('<tr id="file_'+counter+'" rel="'+file.name+'"><td><input type="text" placeholder="Videos Name" id="title'+counter+'" name="report_title'+counter+'" class="form-control required"></td><td><img class="list_image" src="'+file.url+'" /></td><td><a  target="_blank" href="'+file.url+'">'+original_name+'</a></td><td><button type="button" id1="'+counter+'" href="javascript:void(0);" id2="'+file.name+'" id3="'+WWW_ROOT+TEMP_IMG_PATH+file.name+'" class="btn red delete remove_file" rel="'+file.size+'"><i class="fa fa-trash delete_btn"></i><span>'+btn_val+'</span></button></td></tr>');
